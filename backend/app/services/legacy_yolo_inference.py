# """Compatibility shim for the YOLO26 inference service."""
...
class YOLOInferenceService:
    ...
    def _resolve_model_path(self, relative_path: List[str]) -> Optional[Path]:
        ...
        roots = [repo_root, Path("/app"), Path("/app") / "backend"]
        candidates = []
        for root in roots:
            candidates.append(root / "backend" / "models" / Path(*relative_path))
            candidates.append(root / "models" / Path(*relative_path))
            candidates.append(root.joinpath(*relative_path))
        candidates.append(Path(*relative_path))  # bare relative fallback
